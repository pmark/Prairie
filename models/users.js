...
Meteor.methods({
	...
	userNodes: function(cursor) {
		// Return all for now, per prairie later.
		...
		cursor.forEach(function(u,i) {

	        var newNode = {
	            _id: u._id,
	            id: "person-" + u._id,
	            type: "person",
                iconURL: u.profile ? u.profile.iconURL : null,
	            title: u.profile ? u.profile.initials : "???"
	        };
	...
	currentUser: function() {
		return Meteor.users.findOne().fetch();
	},

	iconURLForUser: function(userId) {
		var url = "nada";
		var u = Meteor.users.findOne({_id:userId});

		if (u) {
			if (u.profile.iconURL) {
				url = u.profile.iconURL;
			}
			else if (u.services) {
				if (u.services.github) {

					var res = Meteor.http.get("https://api.github.com/users/" + u.services.github.username).data;
					...
					if (res) {
						...
						Meteor.users.update(u._id, {
							$set: {
								...
								"profile.initials": initials(res.name)
							}
						}, function(err, r) {
							console.log("user update", err, r);
						});
					}
				}
			}
...
function getAvatarFromService(service, userid, size) {
